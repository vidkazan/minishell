...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   builtins_export_sorting.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cstarmie <cstarmie@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/28 08:14:30 by fcody             #+#    #+#             */
...
void	export_sort(t_elem *elem, int write_fd)
{
	...
	while (++count < env_arr_len)
	{
		...
		{
			if (!is_in_export_line(elem->data->envp[i], \
			export_arr) && is_min_arr_line(elem->data->envp[i], \
			elem->data->envp, export_arr))
			{
...
int	export_argument_check(char *arg)
{
	...
	while (*ptr)
	{
		if (!ft_isalpha(*ptr) && !ft_isdigit(*ptr) \
		&& *ptr != '_' && *ptr != '=' && *ptr != '\'' \
		&& *ptr != '\"' && *ptr != ' ' && *ptr != '-')
			return (0);
		ptr++;
